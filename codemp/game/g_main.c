...
#define ARCADE_START_LEVEL 1
#define ARCADE_FINAL_LEVEL 12
#define ARCADE_BOT_LEVEL_OFFSET 2
...
#define ARCADE_CLEANUP_RETRY_BUDGET 20
...
static int G_ArcadeClampPrimaryBotLevel(int arcadeLevel)
{
	if (arcadeLevel < ARCADE_PRIMARY_BOT_LEVEL_MIN)
	{
		return ARCADE_PRIMARY_BOT_LEVEL_MIN;
	}
	if (arcadeLevel > ARCADE_PRIMARY_BOT_LEVEL_MAX)
	{
		return ARCADE_PRIMARY_BOT_LEVEL_MAX;
	}
	return arcadeLevel;
}
...
static int G_ArcadeGetExtraBotCount(int arcadeLevel)
{
	if (arcadeLevel >= 9)
	{
		return 2;
...
void G_ArcadeResetClientRunState(int clientNum)
{
	...
	G_ArcadeSetLastRoundBonuses(clientNum, 0, 0, 0, 0, 0, 0, qfalse);
...
typedef enum
{
	...
	ARCADE_BONUS_TIME,
	ARCADE_BONUS_KILLS,
	ARCADE_BONUS_FLAWLESS,
	ARCADE_BONUS_COUNT
} arcadeRoundBonusType_t;

static const char *G_ArcadeGetRoundBonusLabel(arcadeRoundBonusType_t bonusType)
{
	switch (bonusType)
	{
	case ARCADE_BONUS_BASE:
		return "Base";
	case ARCADE_BONUS_HEALTH:
		return "HP";
	case ARCADE_BONUS_ARMOR:
		return "AP";
	case ARCADE_BONUS_TIME:
		return "Time";
	case ARCADE_BONUS_KILLS:
		return "Kills";
	case ARCADE_BONUS_FLAWLESS:
		return "Flawless";
	default:
		return "";
	}
...
static int G_ArcadeGetLastRoundBonusValue(int clientNum, arcadeRoundBonusType_t bonusType)
{
	switch (bonusType)
	{
	case ARCADE_BONUS_BASE:
		return level.arcadeLastRoundBaseBonus[clientNum];
	case ARCADE_BONUS_HEALTH:
		return level.arcadeLastRoundHealthBonus[clientNum];
	case ARCADE_BONUS_ARMOR:
		return level.arcadeLastRoundArmorBonus[clientNum];
	case ARCADE_BONUS_TIME:
		return level.arcadeLastRoundTimeBonus[clientNum];
	case ARCADE_BONUS_KILLS:
		return level.arcadeLastRoundKillBonus[clientNum];
	case ARCADE_BONUS_FLAWLESS:
		return level.arcadeLastRoundFlawlessBonus[clientNum];
	default:
		return 0;
	}
}
...
static void G_ArcadeSetLastRoundBonuses(int clientNum, int baseBonus, int healthBonus,
	int armorBonus, int timeBonus, int killBonus, int flawlessBonus, qboolean flawlessVictory)
{
	level.arcadeLastRoundBaseBonus[clientNum] = baseBonus;
	level.arcadeLastRoundHealthBonus[clientNum] = healthBonus;
	level.arcadeLastRoundArmorBonus[clientNum] = armorBonus;
	level.arcadeLastRoundTimeBonus[clientNum] = timeBonus;
	level.arcadeLastRoundKillBonus[clientNum] = killBonus;
	level.arcadeLastRoundFlawlessBonus[clientNum] = flawlessBonus;
	level.arcadeLastRoundFlawless[clientNum] = flawlessVictory;
}
...
static void G_ArcadeFormatRoundBreakdown(int clientNum, char *out, int outSize)
{
	...
	for (i = 0; i < ARCADE_BONUS_COUNT; i++)
	{
...
static void G_ArcadeReplayGameOverCenterMessages(void)
{
	...
	for (i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if (!ent->inuse || !ent->client || (ent->r.svFlags & SVF_BOT) ||
			ent->client->pers.connected != CON_CONNECTED || !level.arcadeGameOverRecipient[i])
		{
			continue;
		}
		...
		G_ArcadeSendCenterMessage(ent,
			...
			level.arcadeGameOverComplete ? level.arcadeLastRoundScore[i] : level.arcadeScore[i],
			level.arcadeScore[i], topScore,
			level.arcadeGameOverComplete ? level.arcadeLastRoundFlawless[i] : qfalse);
	}
}
...
static qboolean G_ArcadeTryAddManagedBot(float skill)
{
	...
	trap->Cvar_Set("g_npcspskill", va("%.2f", skill));
...
static int G_ArcadeCountAliveBots(void)
{
	...
	for (i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if (ent->client->sess.sessionTeam == TEAM_SPECTATOR || ent->health < 1)
		{
			continue;
		}
		count++;
...
static void G_ArcadeBeginGameOver(qboolean arcadeComplete)
{
	...
	level.arcadeGameOverCenterReplayTime = level.time + 1000;
	level.arcadeGameOverCenterReplaySent = qfalse;
	level.arcadeGameOverComplete = arcadeComplete;
...
static void G_ArcadeStartRound(void)
{
	...
	const int extraBots = G_ArcadeGetExtraBotCount(level.arcadeLevel);
	...
	targetBots = humans + extraBots;
	if (targetBots > maxManagedBots)
	{
		targetBots = maxManagedBots;
		G_ArcadeWarnNoRoomForPlayers();
	}
	...
	if (G_ArcadeHasAnyBotClients() || G_ArcadeCountManagedBotSlots() > 0)
	{
		...
		{
			level.arcadeCleanupPendingBots = droppedManagedBots;
			level.arcadeCleanupRetryBudget = ARCADE_CLEANUP_RETRY_BUDGET;
	...
	{
		if (level.arcadeCleanupRetryBudget > 0 &&
			availableBotSlots + level.arcadeCleanupPendingBots >= targetBots)
		{
			level.arcadeCleanupPendingBots--;
			level.arcadeCleanupRetryBudget--;
			level.arcadeRoundBotsTarget = targetBots;
			level.arcadeRoundQueuedStart = 0;
			level.arcadeCleanupRetryTime = level.time + ARCADE_CLEANUP_RETRY_DELAY_MS;
			return;
...
void G_ArcadeHandlePlayerDeath(gentity_t *self, gentity_t *attacker)
{
	...
	{
		level.arcadeRoundKills[attacker->s.number]++;
...
static void G_ArcadeFinishRound(qboolean gameOver, qboolean arcadeComplete)
{
	...
	for (i = 0; i < MAX_CLIENTS; i++)
	{
		...
		int killBonus = 0;
		int flawlessBonus = 0;
		qboolean flawlessVictory = qfalse;
		if (!ent->inuse || !ent->client || (ent->r.svFlags & SVF_BOT) ||
			ent->client->pers.connected != CON_CONNECTED)
		{
			continue;
		...
		G_ArcadeSetLastRoundBonuses(i, 0, 0, 0, 0, 0, 0, qfalse);

		if (((!gameOver) || arcadeComplete) &&
			ent->client->sess.sessionTeam != TEAM_SPECTATOR && ent->health > 0)
		{
			const int elapsed = level.time - level.arcadeRoundStartTime;
			const int armorResource = G_ArcadeGetArmorResource(ent);
			timeBonus = G_ArcadeGetTimeBonus(elapsed);
			healthBonus = ent->health * 5;
			armorBonus = armorResource * 10;
			killBonus = level.arcadeRoundKills[i] * 100;
			if (ent->health > 98 && armorResource > 97)
			{
				flawlessVictory = qtrue;
				flawlessBonus = 400 * level.arcadeLevel;
			}
			roundScore = 1000 + healthBonus + armorBonus + timeBonus + killBonus + flawlessBonus;
			level.arcadeLastRoundScore[i] = roundScore;
			G_ArcadeSetLastRoundBonuses(i, 1000, healthBonus, armorBonus, timeBonus, killBonus, flawlessBonus, flawlessVictory);
			level.arcadeScore[i] += roundScore;
			G_ArcadeSyncClientScoreboardScore(ent);
			if (!gameOver)
			{
				G_ArcadeSendCenterMessage(ent, "^2LEVEL CLEAR", roundScore, level.arcadeScore[i], 0, flawlessVictory);
			}
		}
		...
		if (gameOver)
		{
			...
			G_ArcadeSendCenterMessage(ent,
				...
				arcadeComplete ? flawlessVictory : qfalse);
			if (!G_ArcadePlayerIsLoggedIn(ent))
			{
...
static void G_ArcadeRunFrame(void)
{
	const int aliveHumans = G_ArcadeCountAliveHumans();
	const int aliveBots = G_ArcadeCountAliveBots();
	const int managedBotSlots = G_ArcadeCountManagedBotSlots();
	...
	if (level.arcadeGameOverTime &&
		!level.arcadeGameOverCenterReplaySent &&
		level.arcadeGameOverCenterReplayTime > 0 &&
		level.time >= level.arcadeGameOverCenterReplayTime)
	{
		G_ArcadeReplayGameOverCenterMessages();
		level.arcadeGameOverCenterReplaySent = qtrue;
		level.arcadeGameOverCenterReplayTime = 0;
	}
	...
	if (aliveBots <= 0)
	{
		if (managedBotSlots > 0)
		{
			return;
		}
		if (G_ArcadeCountRoundParticipants() <= 0)
		{
			level.arcadeRoundStartTime = 0;
			level.arcadeRoundBotsTarget = 0;
			level.arcadeRoundQueuedStart = level.time + ARCADE_JOIN_QUEUE_DELAY_MS;
		...
		{
			G_ArcadeFinishRound(qfalse, qfalse);
			level.arcadeLevel++;
			level.arcadeRoundQueuedStart = level.time + ARCADE_BETWEEN_LEVEL_DELAY_MS;
		}
		level.arcadeRoundStartTime = 0;
	}
}
