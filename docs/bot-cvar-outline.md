...
| `bot_lowhanginfruitDistance` | `1024` | Max distance to prioritize low-hanging-fruit targets. Note the cvar name is the existing legacy spelling/misspelling and must be used exactly as written. |
...
| `bot_ptkweight_short`, `bot_ptkweight_medium`, `bot_ptkweight_long` | `85`, `150`, `110` | PTK-driven pull selection and its follow-up strongly prefer medium range, where pullkick setups are easiest to stabilize. |
